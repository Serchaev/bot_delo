        ...
        stage('Run Black') {
            steps {
                script {
                    ...
                    sh 'docker run --rm -v $(pwd):/app my-ci-image black --check /app'
                }
            }
        }
        stage('Run Isort') {
            steps {
                script {
                    ...
                    sh 'docker run --rm -v $(pwd):/app my-ci-image isort --check-only /app'
                }
            }
        }
        stage('Run Flake8') {
            steps {
                script {
                    ...
                    sh 'docker run --rm -v $(pwd):/app my-ci-image flake8 /app'
                }
            }
        }
    }
